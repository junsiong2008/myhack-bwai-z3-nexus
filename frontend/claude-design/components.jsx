...
const SectorBadge = ({ sector, size = "sm" }) => {
...
const StageBadge = ({ stage }) => (
  <span className="inline-flex items-center rounded-md font-medium px-2 py-0.5 text-[11px] border"
        style={{ borderColor: "#E0DED8", color: "#6B6A65", background: "#FBFBFA" }}>{stage}</span>
);

const GeoBadge = ({ geo }) => {
  const g = (window.GEOS || []).find(x => x.code === geo) || { flag: "🌐", code: geo };
  return (
    <span className="inline-flex items-center gap-1 rounded-md font-medium px-1.5 py-0.5 text-[11px]"
          style={{ background: "#F0EFEA", color: "#6B6A65" }}>
...
// ───── Score bar ─────
const MatchScoreBar = ({ score, label = "Match score", showLabel = true }) => {
  ...
  return (
    <div>
      ...
      <div className="h-2 rounded-full bg-[#EFEDE6] overflow-hidden">
...
const ProgressBar = ({ value, color = "#185FA5", height = 8, bg = "#EFEDE6" }) => (
...
// ───── Capacity bar (segmented) ─────
const CapacityBar = ({ used, total }) => (
  <div className="flex gap-1">
    {Array.from({ length: total }).map((_, i) => (
      <div key={i} className="h-1.5 flex-1 rounded-full"
           style={{ background: i < used ? "#185FA5" : "#E0DED8" }} />
    ))}
  </div>
);

// ───── KPI Card ─────
const KPICard = ({ label, value, sublabel, Icon: IconC, trend }) => (
  <div className="nx-card p-5 flex flex-col gap-3">
    <div className="flex items-start justify-between">
      ...
      <div className="w-9 h-9 rounded-lg flex items-center justify-center"
           style={{ background: "#F3F1EC", color: "#6B6A65" }}>
...
const ToastProvider = ({ children }) => {
  ...
  const push = React.useCallback((msg, type = "success") => {
    ...
    <ToastCtx.Provider value={{ push }}>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 items-end pointer-events-none">
        {toasts.map(t => {
          ...
          return (
            <div key={t.id} className="nx-toast pointer-events-auto flex items-start gap-3 bg-white border rounded-xl shadow-lg px-4 py-3 max-w-sm"
                 style={{ borderColor: "#E0DED8", borderLeftColor: c.bar, borderLeftWidth: 3 }}>
...
// ───── Empty state ─────
const EmptyState = ({ icon: IconC = Sparkles, title, body }) => (
  <div className="flex flex-col items-center justify-center text-center py-16 px-6">
    <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-3"
         style={{ background: "#F3F1EC", color: "#9C9B96" }}>
      <IconC size={22} />
    </div>
...
Object.assign(window, {
  SECTOR_COLORS, STATUS_STYLES,
  SectorBadge, StatusBadge, StageBadge, GeoBadge, PriorityBadge,
  Avatar, MatchScoreBar, ProgressBar, CapacityBar,
  KPICard, ToastCtx, ToastProvider, useToast,
