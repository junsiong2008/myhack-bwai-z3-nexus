// Programme Pipeline screen
const PipelineScreen = ({ ecosystem, addAssignment, closeProgramme, activateReuseMentors, navigate }) => {
  ...
  const [reuseBannerDismissed, setReuseBannerDismissed] = React.useState(false);

  const reuseEligibleIds = React.useMemo(() => {
    if (!ecosystem.programmeClosed) return [];
    return Object.values(ecosystem.outcomes)
      .filter(o => o.reuse_eligible)
      .map(o => o.mentor_id)
      .filter(Boolean);
  }, [ecosystem.outcomes, ecosystem.programmeClosed]);

  // Derive set of mentor ids who have at least one reuse-eligible outcome
  const reuseMentorSet = React.useMemo(() => {
  ...
  const handleAssignRow = (company, mentorId) => {
    ...
    setOpenRowMenu(null);
    toast.push(`${company.name} assigned to ${MENTORS.find(m => m.id === mentorId).name}`, "success");
  };
    ...
    <div>
        ...
        <div className="mb-4 rounded-xl p-4 flex items-center gap-4 nx-fade-in"
          ...
          <div className="flex -space-x-2 mr-2">
            {Array.from(reuseMentorSet).slice(0, 5).map(mid => {
              const m = MENTORS.find(x => x.id === mid); if (!m) return null;
      ...
      <div className="nx-card p-4 mb-4">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 pr-3 border-r" style={{ borderColor: "#E0DED8" }}>
            <span className="text-[11px] uppercase tracking-wide text-[var(--nx-text-2)] font-medium">Programme</span>
            <button className="nx-input py-1.5 px-2.5 text-[13px] flex items-center gap-2 font-medium">
              ...
              return (
                <button key={s} onClick={onClick}
                        ...
                        style={{
                          ...
                          borderColor: active ? "#185FA5" : "#E0DED8"
                        }}>
                  {s}
                </button>
              );
          ...
          <div className="flex-1 min-w-[180px] relative">
            ...
            <input className="nx-input w-full pl-8" placeholder="Search by company name…"
          ...
          {!ecosystem.programmeClosed && (
            <button onClick={() => setCloseModalOpen(true)}
                    className="nx-btn-outline flex items-center gap-2"
      ...
      <div className="nx-card overflow-hidden">
        <table className="w-full text-[13px]">
          <thead>
            <tr className="text-left" style={{ background: "#FBFBFA", borderBottom: "1px solid #E0DED8" }}>
          ...
          <tbody>
            {filtered.map((c, idx) => {
              ...
              return (
                <tr key={c.id} className="hover:bg-[#FBFBFA] transition"
                    style={{ borderBottom: idx === filtered.length - 1 ? "none" : "1px solid #ECEAE3" }}>
                  <td className="px-4 py-3">
                    <button onClick={() => navigate("matching", { companyName: c.name })}
                            className="font-semibold text-[var(--nx-text)] hover:text-[var(--nx-primary)] hover:underline text-left">
                      {c.name}
                    </button>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      {eng.atRisk && (
                        <span title="At-risk — no sessions in 14+ days"
                              className="inline-flex items-center gap-1 text-[10.5px] font-semibold px-1.5 py-0.5 rounded whitespace-nowrap"
                              style={{ background: "#FCEAEA", color: "#B43938" }}>
                          <AlertTriangle size={9}/> at-risk
                        </span>
                      )}
                      {outcome && outcome.reuse_eligible && (
                        <span title="Mentor flagged reuse-eligible"
                              className="inline-flex items-center gap-1 text-[10.5px] font-semibold px-1.5 py-0.5 rounded whitespace-nowrap"
                  ...
                  <td className="px-3 py-3 text-right pr-4 relative">
                    ...
                    {suggestions && (
                      <div className="absolute right-4 top-full mt-1 z-10 nx-card shadow-lg w-72 text-left">
                        <div className="px-3 py-2 border-b text-[11px] uppercase tracking-wide text-[var(--nx-text-2)] font-medium" style={{ borderColor: "#ECEAE3" }}>
                        ...
                        <div className="px-3 py-1.5 border-t" style={{ borderColor: "#ECEAE3" }}>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7}>
                  <EmptyState icon={Search} title="No companies match" body="Adjust your filters or clear the search." />
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="mt-4 flex items-center justify-between text-[12px] text-[var(--nx-text-3)]">
        ...
        <span>Programme: <b className="text-[var(--nx-text-2)]">CREST 2026 MY</b> · {ecosystem.programmeClosed ? <span className="text-[var(--nx-success)] font-semibold">Closed · outcomes captured</span> : "Active · Last sync 2 min ago"}</span>
      </div>
      ...
      {closeModalOpen && (
        <CloseProgrammeModal
          ...
          onSubmit={(draft) => {
            // Attach mentor_id for downstream usage
            const enriched = {};
