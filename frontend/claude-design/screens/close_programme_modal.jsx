// Close Programme modal — Step 6 NPS + outcome capture
const CloseProgrammeModal = ({ ecosystem, onClose, onSubmit }) => {
  ...
  // Pre-seed outcomes from suggestions
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-6 nx-fade-in"
         style={{ background: "rgba(20, 20, 18, 0.45)" }}>
      <div className="nx-card w-full max-w-[920px] max-h-[88vh] flex flex-col shadow-xl nx-pop">
        {/* Header */}
        <div className="px-6 py-5 border-b flex items-start justify-between" style={{ borderColor: "#ECEAE3" }}>
          <div>
            <div className="text-[11px] uppercase tracking-wider font-medium text-[var(--nx-primary)] mb-1">End of programme</div>
        ...
        {/* Summary bar */}
        <div className="px-6 py-4 grid grid-cols-4 gap-4 border-b" style={{ background: "#FBFBFA", borderColor: "#ECEAE3" }}>
              ...
              {matchedPairs.map(({ company, mentor }) => {
                ...
                return (
                  <div key={company.id} className="rounded-lg border p-3.5 grid grid-cols-[1.1fr_1fr_auto_auto_auto] gap-3 items-center"
                       style={{ borderColor: "#ECEAE3", background: o.outcome === "dropped" ? "#FDF7F6" : "#fff" }}>
                    {/* Company */}
                    ...
                    {/* Outcome toggle */}
                    <div className="flex items-center rounded-md border overflow-hidden text-[11px]" style={{ borderColor: "#E0DED8" }}>
                      <button onClick={() => set(company.id, { outcome: "graduated" })}
                              className="px-2.5 py-1 font-medium"
                              style={{
                                background: o.outcome === "graduated" ? "#1D9E75" : "#fff",
                                color: o.outcome === "graduated" ? "#fff" : "#6B6A65",
                              }}>Graduated</button>
                      <button onClick={() => set(company.id, { outcome: "dropped", reuse_eligible: false })}
                              className="px-2.5 py-1 font-medium border-l"
                              style={{
                                background: o.outcome === "dropped" ? "#E24B4A" : "#fff",
                                color: o.outcome === "dropped" ? "#fff" : "#6B6A65",
                                borderColor: "#E0DED8",
                              }}>Dropped</button>
                    </div>
                    {/* NPS */}
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="mono text-[11px] tabular-nums w-8">{Number(o.mentor_nps).toFixed(1)}</span>
                    </div>
                    {/* Reuse checkbox */}
        ...
        {submitted && (
          <div className="flex-1 flex flex-col items-center justify-center px-8 py-10 text-center">
            ...
            <div className="text-[12px] text-[var(--nx-text-3)] mono mb-4">
              Next: open Programme Pipeline to launch CREST 2027 MY
            </div>
        ...
        {/* Footer */}
        {!submitted && (
          <div className="px-6 py-4 border-t flex items-center gap-3" style={{ borderColor: "#ECEAE3", background: "#FBFBFA" }}>
      ...
      {stars.map(s => {
        ...
        return (
          <button key={s} onClick={() => onChange(s)} className="p-0.5"
                  style={{ color: filled ? "#BA7517" : half ? "#D4A75A" : "#E0DED8" }}>
